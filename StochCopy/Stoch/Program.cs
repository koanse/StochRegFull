...
namespace Stoch
{
    ...
    public class HJInitialParams : IInitialParams
    {
        ...
        double[][] arrCoeff;
        double[] arrUMin, arrUMax, arrUSMin, arrMuOpt, arrSigmaOpt, arrAlpha, arrUMid;
        double r;
        string[,] matrFunc;
        ...
        public HJInitialParams(double xEps, double fEps, double[][] arrCoeff, string[,] matrFunc,
            double[] arrUMin, double[] arrUMax, double[] arrUSMin, double[] arrMuOpt,
            double[] arrSigmaOpt, double[] arrAlpha, double r)
        {
            this.xEpsilon = xEps;
            this.fEpsilon = fEps;
            ...
            this.arrMuOpt = arrMuOpt;
            this.arrSigmaOpt = arrSigmaOpt;
        ...
        {
            int p = arrUMin.Length, n = arrMuOpt.Length;
            double[] arrMu = new double[n];
            for (int j = 0; j < arrMu.Length; j++)
            {
                arrMu[j] = arrCoeff[j][0];
                for (int i = 0; i < p; i++)
                    arrMu[j] += arrCoeff[j][i + 1] * (F(arrUMid[i], matrFunc[i, j]) -
                        f(arrUMid[i], matrFunc[i, j]) * arrUMid[i] + f(arrUMid[i], matrFunc[i, j]) * arrX[i]);
            }
            double[] arrSigma = new double[n];
            for (int j = 0; j < arrMu.Length; j++)
                for (int i = 0; i < p; i++)
                    arrSigma[j] += arrCoeff[j][i + 1] * f(arrUMid[i], matrFunc[i, j]);
            double res = 0, z;
            for (int j = 0; j < arrMu.Length; j++)
                res += arrAlpha[j] * (arrMu[j] - arrMuOpt[j]) * (arrMu[j] - arrMuOpt[j]);
            for (int i = 0; i < p; i++)
            {
            ...
            for (int j = 0; j < n; j++)
                if ((z = arrSigma[j] - arrSigmaOpt[j]) > 0)
                    res += z;
            return res;
        }
